...
#define d_len        30//60 //20  //60   // 70  // 当前位置偏差与第d_len次偏差平均作运算
...
#define cs         45             // 40差速  小转角时没有差速
#define cs1         80             // 80差速  小转角时没有差速
#define cc         1.5             // 1.5差速系数调整
...
uint filter0(uint get_value)
{
	...
	for (count = 1; count < 4; count++)//去掉最大值最小值求平均
...
uint filter1(uint get_value)
{
	...
	for (count = 1; count < 4; count++)//去掉最大值最小值球平均
...
void Speed_Control(void)
{
	...
	if (abs(Servo_Dir - preServo_Dir)<80)
	{
		...
		if (abs(Servo_Dir - servo_mid) >= 450 && sadd_cnt > 7)//死角时的差速
		{
			if (Servo_Dir > servo_mid)
			{
				speed_ept0 = speed_ept0;
...
void SPI0_Init()
{
	...
	SPI0BR = 0x77; //设置SPI波特率，分频系数为2048
	SPI0CR1 = 0x;  //设置SPI工作方式，主机模式，不产生中断，时钟空闲低电平
...
void interrupt 66 PIT0(void)           //pit中断 码盘测得速度值
{
	...
	if (pit_cnt == 2)
	{
		...
		pulse = Pulse_count;
		if (pulse > 112)     //一米大约有11200个脉冲，这里112表示112个脉冲代表1厘米，每有112个脉冲juli加1厘米
		{
			juli++;
			PACNT = 0;
	...
	if (k_cnt == 200)                  //减少中断程序里的运行时间，中断运行时间越短越好
	{
		LCD_P14x16Str(0, 3, "时间");
		LCD_write_shu(35, 4, time2, 4);
		LCD_P14x16Str(0, 24, "距离");
	...
	PITTF_PTF0 = 1;  //pit超时标志寄存器，PTF0表示定时器0，通过写1清零
	time1++;        //时间， time2=time1*1000就是最终在液晶屏上显示的计时时间，周期性中断的定时时间是1ms
